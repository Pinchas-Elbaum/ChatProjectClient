...
interface User {
  ...
  lastActive: string;
  image: string;  //צריך ךראות בהמשך איך להכניס תמונות אם בהרשמה או ידני
}
...
const CommanderDashboard: React.FC = () => {
  ...
  const filteredUsers = users.filter(user => 
    !filterOrg || user.organization.toLowerCase().includes(filterOrg.toLowerCase())
  );
  ...
  return (
    ...
    <div className={styles.dashboard}>
      <h2>כל הנתונים כרגע זה רק דוגמה</h2>
        ...
        <table className={styles.table}>
          ...
          <tbody>
            {filteredUsers.map(user => (
              <tr key={user.id} className={user.threatLevel >= 3 ? styles.highThreat : ''}>
                <td>{user.name}</td>
                ...
                <td>
                  <div className={styles.threatLevel}>
                    {Array.from({ length: 5 }).map((_, i) => (
                      <span 
                        key={i} 
                        className={i < user.threatLevel ? styles.threatActive : styles.threatInactive}
                      >●</span>
                    ))}
                  </div>
                </td>
                <td>{user.suspiciousMessages}</td>
      ...
      <div className={styles.orgStats}>
        ...
        <table className={styles.table}>
          ...
          <tbody>
            {stats.organizationsStats.map(org => (
              <tr key={org.name}>
                <td>{org.name}</td>
                <td>{org.userCount}</td>
                <td>{org.suspiciousCount}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {selectedUser && (
        <div className={styles.modal}>
          <div className={styles.modalContent}>
            ...
            <p>Threat Level: {selectedUser.threatLevel}</p>
            <img 
